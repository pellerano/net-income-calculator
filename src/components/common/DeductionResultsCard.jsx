...
function DeductionsResultsCard({ results, deductions }) {
  const { isr, afp, sfs } = deductions;

  const totalDeductions = useMemo(
    () => parseFloat(isr) + parseFloat(afp) + parseFloat(sfs),
    [isr, afp, sfs]
  );
      ...
      {results ? (
        <>
          <CardHeader className="text-white">
            <CardTitle className="text-center flex flex-col gap-2">
              <p className="text-md font-light">Tu salario bruto seria de:</p>
              <p className="text-4xl font-bold">{formatCurrency(results)}</p>
            </CardTitle>
          </CardHeader>
        ...
        <>
          <CardContent className="flex flex-col gap-4 justify-center items-center h-full pb-6 text-center">
            <Loading />
            <p>Introduce tus deducciones mensuales para ver tus resultados</p>
          </CardContent>
        </>
      )}
    </Card>
...
export default DeductionsResultsCard;
